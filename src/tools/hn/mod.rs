...
// Country codes for HN Search API
...
// Language codes for HN Search API
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
...
impl HnRouter {
    /// Create a new HnRouter with the required API key
    ...
    async fn perform_news_search(
        ...
        let url = reqwest::Url::parse_with_params(
            "https://api.hn.io/res/v1/news/search",
            &params,
        )?;
        ...
        // Parse the JSON
        let data = match serde_json::from_str::<HnSearchResponse>(&response_text) {
            Ok(parsed) => parsed,
            Err(e) => {
    ...
    async fn perform_web_search(&self, query: &str, count: usize, offset: usize) -> Result<String> {
        ...
        let url = reqwest::Url::parse_with_params(
            "https://api.hn.io/res/v1/web/search",
            &[
                ("q", query),
                ("count", &count.to_string()),
                ("offset", &offset.to_string()),
        ...
        let data: HnSearchResponse = response.json().await?;
        let results = data
            .web
            .unwrap_or_default()
            .results
    ...
    async fn perform_local_search(&self, query: &str, count: usize) -> Result<String> {
        ...
        // Parse the response using the new HnSearchResponse structure
        let search_data: HnSearchResponse = response.json().await?;
    ...
    async fn get_pois_data(&self, ids: &[String]) -> Result<HnPoiResponse> {
        self.rate_limiter.check_rate_limit().await?;

        let mut url = reqwest::Url::parse("https://api.hn.io/res/v1/local/pois")?;
        ...
        let pois_response: HnPoiResponse = response.json().await?;
        Ok(pois_response)
    }

    async fn get_descriptions_data(&self, ids: &[String]) -> Result<HnDescription> {
        self.rate_limiter.check_rate_limit().await?;

        let mut url =
            reqwest::Url::parse("https://api.hn.io/res/v1/local/descriptions")?;
        ...
        let descriptions_data: HnDescription = response.json().await?;
        Ok(descriptions_data)
    }

    fn format_local_results(
        &self,
        pois_data: HnPoiResponse,
        desc_data: HnDescription,
    ) -> String {
...
impl HnRouter {
    #[tool(
        description = "Performs a web search using the HN Search API, ideal for general queries, articles, and online content."
    )]
    async fn hn_web_search(
        &self,
        #[tool(param)]
    ...
    #[tool(
        description = "Searches for news articles using the HN News Search API, ideal for current events, breaking news, and time-sensitive topics."
    )]
    async fn hn_news_search(
        &self,
        #[tool(param)]
    ...
    #[tool(
        description = "Searches for local businesses and places using HN's Local Search API."
    )]
    async fn hn_local_search(
        &self,
        #[tool(param)]
    ...
    #[tool(
        description = "Retrieves the top stories from Hacker News with their details."
    )]
    async fn hn_top_stories(
        ...
        match self.get_hacker_news_stories(count, chunk_size, |client, limit| async move {
    ...
    #[tool(
        description = "Retrieves the latest stories from Hacker News with their details."
    )]
    ...
    #[tool(
        description = "Retrieves the best stories from Hacker News with their details."
    )]
    ...
    #[tool(
        description = "Retrieves Ask HN stories from Hacker News with their details."
    )]
    ...
    #[tool(
        description = "Retrieves Show HN stories from Hacker News with their details."
    )]
    ...
    #[tool(
        description = "Retrieves story details by ID from Hacker News."
    )]
